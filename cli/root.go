// Package cli assembles the ossucs command tree from the ossucs
...
// NewApp assembles the kit application from the ossucs domain. The
...
//
// To add a command, declare it in ossucs/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := ossucs.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(ossucs.Domain{}).Register(app)
