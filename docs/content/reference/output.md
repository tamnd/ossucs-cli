...
describes what users get. Pick a format with `-o`, or let ossucs choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ossucs <command> -o table   # aligned columns for reading
ossucs <command> -o jsonl   # one JSON object per line, for piping
ossucs <command> -o json    # a single JSON array
ossucs <command> -o csv     # spreadsheet friendly
ossucs <command> -o tsv     # tab-separated
ossucs <command> -o url     # just the URL column
ossucs <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ossucs <command> --fields id,title,url
```
...
```bash
ossucs <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ossucs <command>            # a table, because this is a terminal
ossucs <command> | wc -l    # JSONL, because this is a pipe
```
